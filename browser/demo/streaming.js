          ...
          success: function () {
            ...
            janus.attach({
              ...
              success: function (pluginHandle) {
                ...
                $("#start")
                  ...
                  .click(function () {
                    ...
                    for (let i in bitrateTimer) clearInterval(bitrateTimer[i]);
                    bitrateTimer = {};
                    janus.destroy();
              ...
              onremotetrack: function (track, mid, on, metadata) {
                ...
                if (!on) {
                  ...
                  if (track.kind === "video") {
                    ...
                    if (remoteVideos === 0) {
                      ...
                      if (
                        $("#" + mstreamId + " .no-video-container").length === 0
                      ) {
                        $("#" + mstreamId).append(
                ...
                if ($("#remotevideo" + mid).length > 0) return;
                ...
                if (track.kind === "audio") {
                  ...
                  $("#" + mstreamId).append(
                    '<video class="rounded centered hide" id="remotevideo' +
                      mid +
                  ...
                  if (!bitrateTimer[mid]) {
                    ...
                    bitrateTimer[mid] = setInterval(function () {
                      if (!$("#remotevideo" + mid).get(0)) return;
        ...
        for (let mp in list) {
          ...
          $("#streamslist").append(
            "<a class='dropdown-item' href='#' id='" +
              list[mp]["id"] +
              "'>" +
              escapeXmlTags(list[mp]["description"]) +
              " (" +
              list[mp]["type"] +
              ")" +
              "</a>",
...
function getStreamInfo() {
  $("#metadata").empty();
  $("#info").addClass("hide");
  if (!selectedStream || !streamsList[selectedStream]) return;
...
function startStream() {
  ...
  if (streamsList[selectedStream].legacy) {
    ...
    if ($("#mstream0").length === 0) {
      ...
      // No remote video yet
      $("#mstream0").append(
        '<video class="rounded centered waitingvideo" id="waitingvideo0" width="100%" height="100%" />',
      );
    }
    dataMid = "0";
  } else {
    ...
    for (let mi in streamsList[selectedStream].media) {
      ...
      if ($("#mstream" + mid).length === 0) {
        ...
        // No remote media yet
        $("#mstream" + mid).append(
          '<video class="rounded centered waitingvideo" id="waitingvideo' +
            mid +
            '" width="100%" height="100%" />',
        );
      }
      if (type === "data") dataMid = mid;
    }
  }
...
function addPanel(panelId, mid, desc) {
  $("#videos").append(
    '<div class="row mb-3" id="panel' +
      ...
      (desc ? desc : "Stream") +
      '				<span class="badge bg-info hide" id="status' +
      mid +
      '"></span>' +
      '				<span class="badge bg-primary hide" id="curres' +
      mid +
      '"></span>' +
      '				<span class="badge bg-info hide" id="curbitrate' +
      mid +
      '"></span>' +
      "			</span>" +
      "		</div>" +
      '		<div class="card-body" id="mstream' +
      panelId +
      '">' +
...
function addSimulcastButtons(mid) {
  $("#curres" + mid)
    ...
    .append(
      '<div id="simulcast' +
        mid +
        '" class="btn-group-vertical btn-group-xs top-right">' +
        '	<div class="btn-group btn-group-xs d-flex" style="width: 100%">' +
        '		<button id="m-' +
        mid +
        '-sl-2" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Switch to higher quality">SL 2</button>' +
        '		<button id="m-' +
        mid +
        '-sl-1" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Switch to normal quality">SL 1</button>' +
        '		<button id="m-' +
        mid +
        '-sl-0" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Switch to lower quality">SL 0</button>' +
        "	</div>" +
        '	<div class="btn-group btn-group-xs d-flex hide" style="width: 100%">' +
        '		<button id="m-' +
        mid +
        '-tl-2" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Cap to temporal layer 2">TL 2</button>' +
        '		<button id="m-' +
        mid +
        '-tl-1" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Cap to temporal layer 1">TL 1</button>' +
        '		<button id="m-' +
        mid +
        '-tl-0" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Cap to temporal layer 0">TL 0</button>' +
        "	</div>" +
        "</div>",
    );
  ...
  $("#m-" + mid + "-sl-0")
    ...
    .click(function () {
      ...
      $("#m-" + mid + "-sl-0")
        ...
        .addClass("btn-info");
      streaming.send({
        message: { request: "configure", mid: mid, substream: 0 },
      });
    });
  $("#m-" + mid + "-sl-1")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-sl-0").hasClass("btn-success"))
        $("#m-" + mid + "-sl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, substream: 1 },
      });
    });
  $("#m-" + mid + "-sl-2")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-sl-0").hasClass("btn-success"))
        $("#m-" + mid + "-sl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, substream: 2 },
      });
    });
  ...
  $("#m-" + mid + "-tl-0")
    ...
    .click(function () {
      ...
      $("#m-" + mid + "-tl-0")
        ...
        .addClass("btn-info");
      streaming.send({
        message: { request: "configure", mid: mid, temporal: 0 },
      });
    });
  $("#m-" + mid + "-tl-1")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-tl-0").hasClass("btn-success"))
        $("#m-" + mid + "-tl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, temporal: 1 },
      });
    });
  $("#m-" + mid + "-tl-2")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-tl-0").hasClass("btn-success"))
        $("#m-" + mid + "-tl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, temporal: 2 },
      });
    });
}
...
function addSvcButtons(mid) {
  ...
  $("#curres" + mid)
    ...
    .append(
      '<div id="svc' +
        mid +
        '" class="btn-group-vertical btn-group-vertical-xs top-right">' +
        '	<div class"row">' +
        '		<div class="btn-group btn-group-xs d-flex" style="width: 100%">' +
        '			<button id="m-' +
        mid +
        '-sl-1" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Switch to normal resolution">SL 1</button>' +
        '			<button id="m-' +
        mid +
        '-sl-0" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Switch to low resolution">SL 0</button>' +
        "		</div>" +
        "	</div>" +
        '	<div class"row">' +
        '		<div class="btn-group btn-group-xs d-flex" style="width: 100%">' +
        '			<button id="m-' +
        mid +
        '-tl-2" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Cap to temporal layer 2 (high FPS)">TL 2</button>' +
        '			<button id="m-' +
        mid +
        '-tl-1" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Cap to temporal layer 1 (medium FPS)">TL 1</button>' +
        '			<button id="m-' +
        mid +
        '-tl-0" type="button" class="btn btn-primary" data-bs-toggle="tooltip" title="Cap to temporal layer 0 (low FPS)">TL 0</button>' +
        "		</div>" +
        "	</div>" +
        "</div>",
    );
  ...
  $("#m-" + mid + "-sl-0")
    ...
    .click(function () {
      ...
      $("#m-" + mid + "-sl-0")
        ...
        .addClass("btn-info");
      streaming.send({
        message: { request: "configure", mid: mid, spatial_layer: 0 },
      });
    });
  $("#m-" + mid + "-sl-1")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-sl-0").hasClass("btn-success"))
        $("#m-" + mid + "-sl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, spatial_layer: 1 },
      });
    });
  $("#m-" + mid + "-tl-0")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      $("#m-" + mid + "-tl-0")
        ...
        .addClass("btn-info");
      streaming.send({
        message: { request: "configure", mid: mid, temporal_layer: 0 },
      });
    });
  $("#m-" + mid + "-tl-1")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-tl-0").hasClass("btn-success"))
        $("#m-" + mid + "-tl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, temporal_layer: 1 },
      });
    });
  $("#m-" + mid + "-tl-2")
    .removeClass("btn-primary btn-success")
    ...
    .click(function () {
      ...
      if (!$("#m-" + mid + "-tl-0").hasClass("btn-success"))
        $("#m-" + mid + "-tl-0")
          ...
          .addClass("btn-primary");
      streaming.send({
        message: { request: "configure", mid: mid, temporal_layer: 2 },
      });
    });
}
